...
/**
 * This service is responsible for causing thumbnail generation to occur,
 * although the actual processing of thumbs occurs elsewhere
 *
 * @author brad
 */
public class ThumbGeneratorService implements Service, CommitListener {
    ...
    public void onCommit(NameNode n) throws Exception {
        try {
            ...
            log.error("Exception in onCommit processing", e);
        }
    }
        ...
        try {
            if (f.isTrash()) {
                ...
                if (f.getParentFolder() != null) {
                    ...
                    if (thumbSpecs == null || thumbSpecs.isEmpty()) {
                        log.trace("not generating thumbnails because there are no thumbspecs");
                        return true;
                    }
                    ThumbnailGeneratorProcessable proc = new ThumbnailGeneratorProcessable(f.getNameNodeId(), f.getName());
                    asynchProc.enqueue(proc);
                    log.trace("enqueued item in ThumbnailGeneratorProcessable: " + f.getClass());
                } else {
                    log.warn("image has no parent folder! " + f.getName());
                }
            }
        } catch (Throwable e) {
            throw new RuntimeException(f.getHref(), e);
        }
        return false;
    }
