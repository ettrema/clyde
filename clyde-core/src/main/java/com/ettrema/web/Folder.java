...
//        return nn;
    ...
    /**
     * Create a new binding (ie LinkedFolder) to this folder in the given
     * destination path and with the given name. The path must be absolute from this host
     * 
     * The path is of the form:
     * /{destination collection}/{destination name}
     * 
     * @param dest 
     */
    public void setNewBinding(String dest) {
        System.out.println("setNewBinding: " + dest);
        LogUtils.trace(log, "setNewBinding", dest);
        Path destPath = Path.path(dest);
        Templatable rDest = this.getHost().find(destPath.getParent());
        if (rDest == null) {
            throw new RuntimeException("Destination folder was not found: " + destPath.getParent());
        } else if (!(rDest instanceof Folder)) {
            throw new RuntimeException("Destination folder isnt actually a folder:  " + destPath.getParent());
        }
        Folder destFolder = (Folder) rDest;
        if (destFolder.hasChild(destPath.getName())) {
            throw new RuntimeException("Destination name already exists: " + destPath.getName() + " in " + destFolder.getHref());
        }
        LinkedFolder lf = new LinkedFolder(destFolder, destPath.getName());
        lf.save();
        lf.setLinkedTo(this);

        // record it on the user so we can show a list of shared folders
        DiscretePrincipal p = getOwner();
        if (p instanceof User) {
            User u = (User) p;
            List<Relationship> rels = this.getNameNode().findToRelations(User.REL_SHARED);
            if (rels == null || rels.isEmpty()) {
                Relationship r = u.getNameNode().makeRelation(this.getNameNode(), User.REL_SHARED);
                u.getNameNode().onNewRelationship(r);
            }
        }
    }
    
    public String getNewBinding() {
        return null; // just here to make newBinding look like a bean property
    }

    public List<LinkedFolder> getBindings() {
        return LinkedFolder.getLinkedDestinations(this);
    }

    public DiscretePrincipal getOwner() {
        Folder f = this;
        while (f != null && !(f instanceof Host)) {
            if (f instanceof DiscretePrincipal) {
                return (DiscretePrincipal) f;
            }
        }
        return null;
    }
