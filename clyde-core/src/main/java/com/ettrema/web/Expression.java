...
public class Expression implements Component, WrappableComponent {
    ...
    /**
     *
     * @param ct - this is the point of reference when evaluating the
     * expression. This might be a template, or is whatever defined the
     * component invoking the expression
     * @param map - maybe null, otherwise is passed to the expression
     * @param expr - the expression to evaluate. This expression is processed by
     * the expression language, NOT by a template interpreter, so does not
     * contain template tags like $ and @
     * @param targetPage - is passed to the expression as targetContainer.
     * Should be the actual target of the request
     * @return
     */
    ...
    public static Object doCalc(Object ct, Map map, String expr) {
        ...
        try {
            ...
            if (ct == null) {
                throw new RuntimeException("Target object is null. Expression: " + expr, e);
            } else {
                throw new RuntimeException("Target object: " + ct.getClass() + "  Expression: " + expr, e);
            }
        }
    ...
    /**
     * Calculate this expression, using the given resource as the point of
     * reference
     *
     * @param ct
     * @return
