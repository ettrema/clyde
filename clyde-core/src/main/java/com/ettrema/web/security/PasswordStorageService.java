 ...
 *
 * Supports the old style of password storage which was to store a Text
 * component on the user, as well as the newer style which is to rely on
 * component definitions to set component values
 *
...
public class PasswordStorageService {
    ...
    /**
     * First look for a component value and use it if exists. If not, look for
     * the locally defined Text component
     *
     * @return
     */
    ...
    public void setPasswordValue(User user, String newPassword) {
        ...
        if (enableValidation) {
            validationErr = passwordValidationService.checkValidity(user, newPassword);
        } else {
            validationErr = null;
        }
        ComponentValue cv;
