...
//        template = "<acronym title='${path}'>" + template + "</acronym>";
    ...
    public String render(ComponentValue c, RenderContext rc) {
        ...
        if (!disAllowTemplating) {
            ...
            String result = _render(template, vc);
            if (log.isTraceEnabled()) {
    ...
    public String getCustomValidationMessage() {
        return customValidationMessage;
    }

    public void setCustomValidationMessage(String customValidationMessage) {
        this.customValidationMessage = customValidationMessage;
    }

    public void setValidationMessage(ComponentValue cv, String defaultMessage) {
        if( customValidationMessage != null && customValidationMessage.length() > 0 ) {
            cv.setValidationMessage(customValidationMessage);
        } else {
            cv.setValidationMessage(defaultMessage);
        }
    }
}
