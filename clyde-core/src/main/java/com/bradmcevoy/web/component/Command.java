...
//        Map<String,String> map = new HashMap<String, String>();
    ...
    public String process(CommonTemplated targetPage) throws NotAuthorizedException {
        RenderContext rc = new RenderContext(null, targetPage, null, false);
        if (!validate(rc)) {
            log.debug("validation failed");
            return null;
        }
    ...
    public Object evaluate(RenderContext rc, Addressable from) {
        ...
        String newHref;
        try {
            newHref = doProcess(rc, null, null);
        } catch (NotAuthorizedException ex) {
            throw new RuntimeException(ex);
        }
        return newHref;
