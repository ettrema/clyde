...
public class Group extends Folder implements Mailbox {
    ...
    public Folder getMailFolder(String name, boolean create) {
        ...
        if( emailFolder == null && create ) {
            try {
                ...
                throw new RuntimeException( "Cant create " + emailFolderName + " in " + this.getHref(), ex);
            }
        }
        return emailFolder;
    }
    ...
    public void storeMail(MimeMessage mm) {
        ...
        if( mailProc == null ) throw new RuntimeException("No " + MailProcessor.class.getCanonicalName() + " is configured. Check catalog.xml");
