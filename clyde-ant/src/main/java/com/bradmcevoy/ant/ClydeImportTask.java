...
public class ClydeImportTask extends AbstractClydeTask {
    ...
    private void importFolder(Folder folderDataNode, File folder) {
        if( !folder.isDirectory() ) throw new IllegalArgumentException("not a folder: " + folder.getAbsolutePath());
    ...
    class Importer extends FrameworkBase {
        ...
        BaseResource go() {
            ...
            if( res instanceof BinaryFile ) {
                ...
                if( f.exists() ) {
                    ...
                    if( f.length() != sz ) throw new RuntimeException("file sizes dont match: " + f.length() + " != " + sz);
